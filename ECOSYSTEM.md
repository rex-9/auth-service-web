...
- **Asset Management**: `GET/PUT/DELETE /v1/admin/assets` (CRUD + upload + discard/undiscard/destroy, search, filter by type/format/source including `subtitle`, dynamic in-place S3 rename on type update, `POST /v1/admin/assets/:id/thumbnail/upload` and `POST /v1/admin/assets/:id/thumbnail/regenerate`, `POST /v1/admin/assets/:id/subtitle/upload` for `.srt` sidecars on video/audio, `GET /v1/admin/assets/storage_stats` for Garage bucket & VPS disk metrics, real-time ActionCable compression and `asset_thumbnail_generated` updates, secondary compression pass trigger with 2-pass safeguard).
...
- **Asset Control Center**: Dedicated operational asset management under `/admin/assets`. Features a live Storage & VPS Capacity dashboard (`AdminAssetStorageStats`) showing real-time Garage bucket occupied space, object count, and host VPS disk capacity with low-disk alerts; a multi-file batch upload dialog with optimistic row prepending; out-of-order socket reconciliation (`pendingSocketUpdates`); real-time compression badges (`optimal`, `ready`, `processing`, `pending`); disabled action buttons during in-flight processing; manual secondary compression pass triggers; video/audio thumbnail replace; and `.srt` subtitle upload (`POST /v1/admin/assets/:id/subtitle/upload`) with nested `IChildAsset` display on detail and edit.
